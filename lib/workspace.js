...
function buildFromEnv() {
  ...
  const DB_VARS = ['DB_PROFILE','DB_EXPENSES','DB_CONTRACTS','DB_SIGNALS',
                   ...
                   'DB_ENGAGEMENTS','DB_OUTREACH'];
  for (const v of DB_VARS) {
    if (!process.env[v]) console.warn(`  ⚠  ${v} not set in .env — that endpoint will fail`);
  }
  // Scenarios DBs are optional (created by setup-scenarios.js)
  if (!process.env.DB_SCENARIOS) {
    console.warn('  ℹ  DB_SCENARIOS not set — run setup-scenarios.js to enable Scenario Planner');
  }
    ...
    workspaces: {
      primary: {
        ...
        dbs: {
          ...
          scenarioProjections:  process.env.DB_SCENARIO_PROJECTIONS || '',
          engagements:          process.env.DB_ENGAGEMENTS          || '',
          outreach:             process.env.DB_OUTREACH             || '',
        },
      },
    },
  };
