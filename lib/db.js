...
/** Hash a magic-link token before storing / querying — tokens are never stored plain. */
...
const users = {
  ...
  /** Find user by magic token ignoring expiry — used to distinguish "expired" from "invalid". */
  findExpiredMagicToken(token) {
    const hashed = _hashToken(token);
    return get('SELECT * FROM users WHERE magic_token = ?', [hashed]);
  },

  /**
   * Rate-limit check: returns true if a new magic link may be sent.
   * Enforces a 60-second cooldown between sends (based on TTL remaining vs issued time).
   */
  canSendMagicLink(userId, ttl = 900) {
    const user = users.findById(userId);
    if (!user?.magic_expires) return true;
    const now = Math.floor(Date.now() / 1000);
    // If (magic_expires - now) > (ttl - 60), the last link was sent < 60s ago
    return (user.magic_expires - now) < (ttl - 60);
  },
  ...
  setMagicToken(userId, token, expiresInSeconds = 900) {
    const expires = Math.floor(Date.now() / 1000) + expiresInSeconds;
    // Store SHA-256 hash — never store the raw token
    run('UPDATE users SET magic_token=?, magic_expires=? WHERE id=?',
        [_hashToken(token), expires, userId]);
    _persist();
  },
