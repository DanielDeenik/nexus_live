...
function initLinkedIn() {
  ...
  // passport-linkedin-oauth2 v2 calls /v2/me (old API) but openid/profile/email
  // scopes issue OIDC tokens that only work with /v2/userinfo. Override it.
  linkedInStrategy.userProfile = function(accessToken, done) {
    ...
    this._oauth2.get('https://api.linkedin.com/v2/userinfo', accessToken, (err, body) => {
      if (err) return done(new Error('Failed to fetch LinkedIn profile: ' + err.message));
      try {
        const json = JSON.parse(body);
        const profile = {
          provider:     'linkedin',
          id:           json.sub,
          displayName:  json.name || [json.given_name, json.family_name].filter(Boolean).join(' '),
        ...
        done(new Error('Failed to parse LinkedIn profile: ' + e.message));
      }
    });
  };
