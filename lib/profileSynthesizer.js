 ...
 *
 * @param {object|null} linkedin  — from LinkedIn OAuth: { name, headline, email, picture, country }
...
function synthesize(linkedin, cv, sows = []) {
  ...
  const location = li.country  || c.location || null;
  const picture  = li.picture  || null;

  // ── Skills (CV primary — LinkedIn basic scope doesn't return skills) ─────
  const skills = dedupInsensitive(c.skills || []);
  ...
  // ── Inferred metadata ────────────────────────────────────────────────────
  const seniority    = inferSeniority(headline, c.yearsExperience);
  const companySizes = inferCompanySize(sowClientTypes);
  const yearsExp     = c.yearsExperience || null;
  ...
  // ── Sources summary ───────────────────────────────────────────────────────
  const sources = [];
  if (li.name)            sources.push('LinkedIn');
